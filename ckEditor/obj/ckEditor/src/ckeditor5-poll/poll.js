...
import { POLL_CLASS } from "./constants";
const DEFAULT_PROPS = {
...
function updateViewElementText(viewWriter, rootViewElement, classPath, newText) {
  if (classPath.length < 1) return;
  const [headClass, ...tail] = classPath;
  const targetElement = Array.from(rootViewElement.getChildren()).find((child) =>
    child.hasClass(headClass),
  );
  ...
  _defineConverters() {
    ...
    editor.conversion.for("editingDowncast").attributeToElement({
      ...
      view: (props, { writer: viewWriter }, data) => {
        if (!props) return;
        const pollModelElement = data.item;
