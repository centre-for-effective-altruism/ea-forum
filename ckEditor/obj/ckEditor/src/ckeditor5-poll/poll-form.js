...
export const POLL_COLOR_SCHEMES = [
...
class MainFormView extends View {
  ...
  _createInputs() {
    ...
    questionView.label = "Question";
    questionView.on("input", () => {
      const model = this.editor.model;
      const selectedElement = this.selectedElement;
      model.change((writer) => {
        if (!selectedElement) return;
        const props = this.selectedElement.getAttribute("props");
        const inputValue = questionView.element.value.trim();
        writer.setAttribute(
          "props",
          { ...props, question: inputValue },
    ...
    agreeWordingView.label = "Right label";
    ...
    disagreeWordingView.setTemplate({
      ...
      attributes: {
        ...
        class: [
          "ck-poll-form-input",
          disagreeWordingBind.if("hasError", "ck-error"),
        ],
    ...
    disagreeWordingView.label = "Left label";
    disagreeWordingView.on("input", () => {
      ...
      model.change((writer) => {
        ...
        writer.setAttribute(
          "props",
          { ...props, disagreeWording: inputValue },
          this.selectedElement,
        );
      });
    });
    ...
    const colorSchemeButtons = POLL_COLOR_SCHEMES.map((colorScheme, index) => {
      ...
      buttonView.on("execute", () => {
        ...
        model.change((writer) => {
          ...
          writer.setAttribute(
            "props",
            { ...props, colorScheme: newColorScheme },
            this.selectedElement,
          );
    ...
    const createDurationInput = (label, initialValue = 0) => {
      ...
      inputView.on("input", () => {
        ...
        if (!selectedElement) return;
        model.change((writer) => {
          const props = selectedElement.getAttribute("props");
...
export default class PollForm extends Plugin {
  ...
  _createFormView() {
    ...
    formView.keystrokes.set("Enter", (_, cancel) => {
  ...
  _addFormView(selectedElement) {
    if (this._isFormInPanel) {
      ...
      const pollProps = selectedElement.getAttribute("props");
      ...
      this.formView.disagreeWordingView.element.value =
        pollProps.disagreeWording || "";
      ...
      const currentIndex = POLL_COLOR_SCHEMES.findIndex(
        (cs) =>
          cs.darkColor ===
            (currentColorScheme === null || currentColorScheme === void 0
              ? void 0
              : currentColorScheme.darkColor) &&
          cs.lightColor ===
            (currentColorScheme === null || currentColorScheme === void 0
              ? void 0
              : currentColorScheme.lightColor),
      );
...
//# sourceMappingURL=poll-form.js.map
