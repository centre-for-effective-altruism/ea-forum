...
export default class CollapsibleSections extends Plugin {
  ...
  init() {
    ...
    this.editor.commands.add(
      ...
      new InsertCollapsibleSectionCommand(this.editor),
    );
    this.editor.ui.componentFactory.add("collapsibleSectionButton", (locale) => {
      ...
      this.listenTo(buttonView, "execute", () =>
        this.editor.execute("insertCollapsibleSection"),
      );
      return buttonView;
    });
  }
  ...
  _defineConverters() {
    ...
    conversion.for("editingDowncast").elementToStructure({
      ...
      view: (modelElement, conversionApi) => {
        ...
        const collapsibleSectionElement = writer.createEditableElement("div", {
          class: "detailsBlock detailsBlockEdit",
        });
        const titleElement = conversionApi.writer.createEditableElement("div", {
          class: "detailsBlockTitle",
        });
        const detailsElement = writer.createEditableElement("div", {
          class: "detailsBlockContent",
        });
        writer.insert(
          writer.createPositionAt(collapsibleSectionElement, 0),
          titleElement,
        );
    ...
    conversion.for("dataDowncast").elementToStructure({
      ...
      view: (modelElement, conversionApi) => {
        const { writer } = conversionApi;
        const collapsibleSectionElement = writer.createEditableElement("details", {
          class: "detailsBlock",
        });
        const titleElement = conversionApi.writer.createEditableElement("summary", {
          class: "detailsBlockTitle",
        });
        const detailsElement = writer.createEditableElement("div", {
          class: "detailsBlockContent",
...
export class InsertCollapsibleSectionCommand extends Command {
  execute() {
    ...
    model.change((writer) => {
      ...
      const { collapsibleSection, collapsibleSectionTitle } =
        createCollapsibleSection(writer);
  ...
  refresh() {
    ...
    const allowedIn =
      lastPosition &&
      model.schema.findAllowedParent(lastPosition, "collapsibleSection");
...
//# sourceMappingURL=collapsible-sections-plugin.js.map
