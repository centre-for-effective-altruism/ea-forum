...
export const addFootnoteAutoformatting = (editor, rootElement) => {
  if (editor.plugins.has("Autoformat")) {
    ...
    inlineAutoformatEditing(
      ...
      (_, ranges) => formatCallback(ranges, editor, rootElement),
    );
  }
};
/**
...
const regexMatchCallback = (editor, text) => {
  ...
  const surroundingText =
    selectionStart &&
    (selectionStart.textNode || selectionStart.getShiftedBy(-1).textNode);
  ...
  for (const result of results || []) {
    ...
    const textNodeOffset =
      selectionStart.parent.getChildStartOffset(surroundingText);
    // if the cursor isn't at the end of the range to be replaced, do nothing
    if (
      textNodeOffset === null ||
      selectionStart.offset !== textNodeOffset + removeEndIndex
    ) {
      continue;
    }
...
//# sourceMappingURL=autoformatting.js.map
