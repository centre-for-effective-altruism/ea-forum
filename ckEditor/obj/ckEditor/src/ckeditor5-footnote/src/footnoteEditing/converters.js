...
export const defineConverters = (editor) => {
  ...
  conversion.for("editingDowncast").elementToElement({
    ...
    view: (modelElement, conversionApi) => {
      ...
      const footnoteReferenceViewElement = createFootnoteReferenceViewElement(
        modelElement,
        conversionApi,
      );
  ...
  conversion.for("editingDowncast").add((dispatcher) => {
    dispatcher.on(
      ...
      (_, data, conversionApi) =>
        updateFootnoteReferenceView(data, conversionApi, editor),
      { priority: "high" },
    );
  });
...
function createFootnoteReferenceViewElement(modelElement, conversionApi) {
  ...
  viewWriter.insert(
    viewWriter.createPositionAt(footnoteReferenceView, 0),
    superscript,
  );
...
function updateFootnoteReferenceView(data, conversionApi, editor) {
  ...
  viewWriter.setAttribute(
    "href",
    `#fn${item.getAttribute(ATTRIBUTES.footnoteId)}`,
    anchor,
  );
