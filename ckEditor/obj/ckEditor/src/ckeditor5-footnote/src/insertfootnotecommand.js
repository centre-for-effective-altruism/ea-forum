...
export default class InsertFootnoteCommand extends Command {
  ...
  execute({ footnoteIndex } = { footnoteIndex: 0 }) {
    this.editor.model.enqueueChange((modelWriter) => {
      ...
      const footnoteReference = modelWriter.createElement(
        ELEMENTS.footnoteReference,
        { [ATTRIBUTES.footnoteId]: id, [ATTRIBUTES.footnoteIndex]: index },
      );
  ...
  refresh() {
    ...
    const allowedIn =
      lastPosition &&
      model.schema.findAllowedParent(lastPosition, ELEMENTS.footnoteSection);
...
//# sourceMappingURL=insertfootnotecommand.js.map
