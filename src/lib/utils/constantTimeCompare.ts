export const constantTimeCompare = ({
  ...
  try {
    ...
    // Iterate over the array of correct characters, which has a known
    // (constant) length, to mitigate certain timing attacks
    for (const [idx, char] of Object.entries(correctValueChars)) {
      const matchedIndexCharsEqual = char === unknownValueChars[Number(idx)];
