...
import {
  getSearchIndexPrefix,
  SearchOptions,
  SearchQuery,
} from "@/lib/search/searchClient";
import type { SearchResult } from "@/lib/search/SearchResult";
import type { SearchDocument } from "@/lib/search/searchDocuments";
import { indexNameToConfig } from "./ElasticConfig";
import { getElasticOriginDate } from "./elasticSettings";
import { QueryFilter, QueryFilterOperator } from "./ElasticQuery";
...
class ElasticService {
  ...
  async runQuery(
    ...
    return {
      ...
      serverTimeMS: timeMS,
      /**
       * `facets` and `facets_stats` are needed to tell the frontend it should
       * enable controls for choosing facets (most notably, publicDateMs).
       */
      facets: {
        publicDateMs: {},
      },
      facets_stats: {
        publicDateMs: {
          min: getElasticOriginDate().getTime(),
  ...
  private sanitizeIndexName(indexName: string): SanitizedIndexName {
    const prefix = getSearchIndexPrefix();
    if (prefix) {
