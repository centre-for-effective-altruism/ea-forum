...
const setCountryCodeToLocalStorage = (countryCode: string) => {
  const ls = getBrowserLocalStorage();
...
const getCachedUserCountryCode = () => {
  if (isServer) {
    return null;
  }
...
export const getExplicitConsentRequiredAsync = async (): Promise<
  boolean | "unknown"
> => {
  if (isServer) {
    return "unknown";
...
export const getExplicitConsentRequiredSync = (): boolean | "unknown" => {
  if (isServer) {
    return "unknown";
  }
  const cachedCountryCode = getCachedUserCountryCode();
