...
export type SearchableMultiSelectState = MultiSelectState & {
  /**
   * If the user enters a search, selects an option, then searches for
...
export const useSearchableMultiSelect = ({
  ...
  const filteredSelectedValues = suggestions
  ...
  const hitToSuggestion = useCallback(
    (hit: string, selected = false) => ({
      value: hit,
      label: hit,
      selected,
  ...
  const grandfatheredCount = suggestions.filter(
