...
export const fetchNotificationDisplays = async ({
  userId,
  type,
  ...
  const result = await db.execute<NotificationDisplay>(sql`
    ...
    SELECT
      ...
      tr."_id" "tagRelId",
      COALESCE(
        ${sql.raw(buildNotificationPost("p", "pr", "pu", "pl"))},
        ${sql.raw(buildNotificationPost("trp", "trpr", "trpu", "trpl"))}
      ) "post",
      ${sql.raw(buildNotificationComment("c", "cu", "cp", "cpr", "cpu", "cpl"))} "comment",
      ${sql.raw(buildNotificationTag("t"))} "tag",
      ${sql.raw(buildNotificationSequence("s"))} "sequence",
      COALESCE(
        ${sql.raw(buildNotificationUser("nma"))},
        ${sql.raw(buildNotificationUser("u"))}
      ) "user",
      ${sql.raw(buildNotificationLocalgroup("l"))} "localgroup"
    ...
    WHERE
      ...
      ${type ? `${sql.raw(`n."type" =`)} ${type} ${sql.raw("AND")}` : sql.raw("")}
      ${sql.raw(
...
export const fetchUnreadNotificationCount = async (user: CurrentUser) => {
  const result = await db.query.notifications.findFirst({
    ...
    where: {
      ...
      deleted: { ne: true },
      emailed: { ne: true },
      waitingForBatch: { ne: true },
      ...(user.lastNotificationsCheck && {
        createdAt: { gt: user.lastNotificationsCheck },
      }),
    },
  });
