...
> extends Watchdog {
   ...
   */
  constructor(
    Editor: { create(...args: any): Promise<TEditor> } | null,
    ...
    this._throttledSave = throttle(
      ...
      typeof watchdogConfig.saveInterval === "number"
  ...
  protected override _restart(): Promise<unknown> {
    return Promise.resolve()
      ...
      .then(() => {
        ...
        const oldRootsAttributes: Record<string, RootAttributes> =
          this._config!.rootsAttributes || {};
  ...
  public create(
    elementOrData:
      ...
      | string
      | Record<string, string>
      | Record<string, HTMLElement> = this._elementOrData!,
    ...
    return Promise.resolve()
      .then(() => {
        ...
        this._initUsingData =
          typeof elementOrData == "string" ||
          (Object.keys(elementOrData).length > 0 &&
            typeof Object.values(elementOrData)[0] == "string");
  ...
  private _getData(): EditorData {
    const editor = this._editor!;
    const roots = editor.model.document.roots.filter(
      (root) => root.isAttached() && root.rootName != "$graveyard",
    );
    ...
    const commentsRepository =
      ...
      (plugins.get("CommentsRepository") as any);
    const trackChanges =
      plugins.has("TrackChanges") && (plugins.get("TrackChanges") as any);
...
class EditorWatchdogInitPlugin {
  ...
  private _restoreCollaborationData() {
    ...
    parsedCommentThreads.forEach((commentThreadData) => {
      ...
      const commentsRepository = this.editor!.plugins.get(
        "CommentsRepository",
      ) as any;

      if (commentsRepository.hasCommentThread(commentThreadData.threadId)) {
        const commentThread = commentsRepository.getCommentThread(
          commentThreadData.threadId,
        )!;
