...
import type {
  ContextWatchdogItemErrorEvent,
  ContextWatchdogItemRestartEvent,
} from "./contextwatchdog";
...
export default abstract class Watchdog {
  ...
  constructor(config: WatchdogConfig) {
    ...
    this._crashNumberLimit =
      typeof config.crashNumberLimit === "number" ? config.crashNumberLimit : 3;
    this._minimumNonErrorTimePeriod =
      typeof config.minimumNonErrorTimePeriod === "number"
   ...
   * Note that this method differs from the CKEditor 5's default `EventEmitterMixin` implementation.
   */
  protected _fire<K extends keyof EventMap>(
    eventName: K,
    ...args: EventArgs<K>
  ): void {
  ...
  private _shouldRestart(): boolean {
    ...
    const firstMeaningfulErrorTime =
      this.crashes[this.crashes.length - 1 - this._crashNumberLimit].date;

    const averageNonErrorTimePeriod =
      (lastErrorTime - firstMeaningfulErrorTime) / this._crashNumberLimit;
