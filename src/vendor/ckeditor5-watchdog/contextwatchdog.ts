...
import Watchdog, { WatchdogConfig, WatchdogState } from "./watchdog";
import EditorWatchdog, { EditorCreatorFunction } from "./editorwatchdog";
...
> extends Watchdog {
        ...
        return this._actionQueues.enqueue(item.id, () => {
          ...
          if (item.type === "editor") {
            ...
            watchdog.on("error", (evt, { error, causesRestart }) => {
              ...
              void this._actionQueues.enqueue(
                item.id,
                () =>
  ...
  private _destroy(): Promise<unknown> {
    return Promise.resolve().then(() => {
      ...
      return (
        Promise.all(
          Array.from(this._watchdogs.values()).map((watchdog) => watchdog.destroy()),
        )
