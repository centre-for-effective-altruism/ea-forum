...
function calculateOperations(
  ...
  matches.push(
    new (<any>Match)(beforeTokens.length, afterTokens.length, 0, segment),
  );
  ...
  for (var index = 0; index < matches.length; index++) {
    ...
    if (actionUpToMatchPositions !== "none") {
      operations.push({
        ...
        endInBefore:
          actionUpToMatchPositions !== "insert" ? match.startInBefore - 1 : null,
...
var OPS: any = {
  ...
  replace: function () {
    return (
      OPS["delete"].apply(null, arguments) + OPS["insert"].apply(null, arguments)
    );
  },
};
