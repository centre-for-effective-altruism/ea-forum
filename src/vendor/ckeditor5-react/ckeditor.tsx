...
const REACT_INTEGRATION_READ_ONLY_LOCK_ID =
  "Lock from React integration (@ckeditor/ckeditor5-react)";

export default class CKEditor<TEditor extends Editor> extends React.Component<
  Props<TEditor> & {
...
> {
   ...
   */
  private watchdog: EditorWatchdog<TEditor> | EditorWatchdogAdapter<TEditor> | null =
    null;
  ...
  private async _initializeEditor(): Promise<unknown> {
    ...
    if (this.props.disableWatchdog) {
      this.instance = await this._createEditor(
        this.domContainer.current!,
        this._getConfig(),
      );
      return;
    }
    ...
    if (this.context instanceof ContextWatchdog) {
      ...
      this.watchdog = new CKEditor._EditorWatchdog(
        this.props.editor,
        this.props.watchdogConfig,
      );
    }
  ...
  private _createEditor(
    element: HTMLElement | string | Record<string, string>,
    config: EditorConfig,
  ): Promise<TEditor> {
    return this.props.editor
      .create(element as HTMLElement, config)
      .then((editor) => {
  ...
  public static propTypes = {
    editor: PropTypes.func.isRequired as unknown as Validator<{
 ...
 */
interface Props<TEditor extends Editor> extends InferProps<
...
export class EditorWatchdogAdapter<TEditor extends Editor> {
  ...
  public on(
    _: string,
    callback: (_: null, data: { error: Error; causesRestart?: boolean }) => void,
  ): void {
