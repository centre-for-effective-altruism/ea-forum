...
import { getSocialImagePreviewPrefix } from "@/lib/posts/postsHelpers";
import {
...
describe("magazine post images", () => {
  ...
  test("prefers an existing manual social preview image", () => {
    ...
    ).toBe(`${getSocialImagePreviewPrefix()}manual/social-preview`);
  });
  test("prefers an existing event image", () => {
    ...
    ).toBe(`${getSocialImagePreviewPrefix()}events/event-image`);
  });
  test("prefers an existing automatically generated social preview image", () => {
    const autoImageUrl = `${getSocialImagePreviewPrefix()}auto/social-preview`;
    expect(
      getMagazinePostImageUrl({
        ...basePost,
  ...
  test("returns a default cloudinary image when the post has no social preview", () => {
    ...
    expect(imageUrl).toMatch(/^.*\/SocialPreview\/defaults\//);
  });
